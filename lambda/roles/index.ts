...
export const handler = async (event: AppSyncResolverEvent<any>) => {
  ...
  if (field === "setUserRole") {
    ...
    // Guard: only ADMIN can update others; anyone can update their own record
    ...
    const item: User = existing.Item
      ? {
          ...
          email:
            (input.email ?? (existing.Item as User).email ?? null)?.toLowerCase?.() ??
            null,
          role: input.role,
  ...
  // NEW: simple admin listing with optional email substring search + pagination
