...
/* build dependent endpoints */
const cognitoDomain = `https://${domain}.auth.${region}.amazoncognito.com`;
const appsyncUrl  = pick('appsyncUrl');
const uploadsApi  = (pick('uploadsApiUrl','uploadsUrl','apiUrl') || '').replace(/\/+$/,'');
...
async function buildLoginUrl(){
  ...
  const p=new URLSearchParams({
    response_type:'code', client_id:clientId, redirect_uri:String(redirectUri||''),
...
q('#signout').addEventListener('click', (e)=>{ e.preventDefault(); sessionStorage.clear(); setAuthUi(); renderNavForRole(null); });
q('#signout-global').addEventListener('click', (e)=>{
  e.preventDefault(); sessionStorage.clear(); renderNavForRole(null);
  const url = `${cognitoDomain}/logout?client_id=${encodeURIComponent(String(clientId||''))}&logout_uri=${encodeURIComponent(String(logoutUri||''))}`;
  location.assign(url);
});
...
async function gql(query, variables={}){
  ...
  const r=await fetch(String(appsyncUrl||''),{method:'POST',headers:{'Content-Type':'application/json',Authorization:id},body:JSON.stringify({query,variables})});
...
/* ===== Role-aware Creator link in the top nav ===== */

function renderNavForRole(profile) {
  try {
    const link = document.getElementById('creatorLink');
    if (!link) return; // page might not have the link
    const role = profile?.role;
    // Show for creators and admins
    const show = role === 'CREATOR' || role === 'ADMIN';
    link.style.display = show ? '' : 'none';
  } catch (_) {}
}

// Boot-time: load { me } and toggle the nav link
(async function initFan() {
  try {
    const d = await gql('{ me { id email role tier createdAt updatedAt } }');
    const me = d?.me;
    renderNavForRole(me);
  } catch (e) {
