...
var WEB_ORIGIN = process.env.WEB_ORIGIN || "";
function pickOrigin(h) {
  if (!h) return void 0;
  const origin = h["origin"] || h["Origin"];
  if (!origin) return void 0;
  const allow = /* @__PURE__ */ new Set([WEB_ORIGIN, "http://localhost:5173"]);
  return allow.has(origin) ? origin : void 0;
}
...
var ok = (data, statusCode = 200, origin) => ({
  statusCode,
  headers: { "Content-Type": "application/json", ...corsHeaders(origin) },
  body: json(data)
});
var txt = (text = "", statusCode = 200, origin) => ({
  statusCode,
  headers: corsHeaders(origin),
  body: text
});
var err = (message, statusCode = 500, origin) => ok({ message }, statusCode, origin);
