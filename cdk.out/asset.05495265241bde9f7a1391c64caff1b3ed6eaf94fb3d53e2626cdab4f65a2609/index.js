...
var thumbnailer_exports = {};
__export(thumbnailer_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(thumbnailer_exports);
var import_client_s3 = require("@aws-sdk/client-s3");
var import_sharp = __toESM(require_lib());
var s3 = new import_client_s3.S3Client({});
var BUCKET = process.env.BUCKET;
var OUT_PREFIX = process.env.THUMBS_PREFIX || "thumbnails/";
var MAX_W = parseInt(process.env.MAX_WIDTH || "360", 10);
var QUALITY = parseInt(process.env.JPEG_QUALITY || "80", 10);
...
var handler = async (event) => {
  for (const rec of event.Records) {
    const { key } = JSON.parse(rec.body);
    console.log("Thumb job for key:", key);
    const obj = await s3.send(new import_client_s3.GetObjectCommand({ Bucket: BUCKET, Key: key }));
    const body = obj.Body;
    const buf = await streamToBuffer(body);
    const thumb = await (0, import_sharp.default)(buf).resize({ width: MAX_W, withoutEnlargement: true }).jpeg({ quality: QUALITY }).toBuffer();
