...
var handler = async (event) => {
  try {
    ...
    if (!approvalId || !decision) {
      return {
        statusCode: 400,
        body: JSON.stringify({ message: "Missing approvalId or decision" })
      };
    }
    if (decision !== "APPROVE" && decision !== "REJECT") {
      return {
        statusCode: 400,
        body: JSON.stringify({ message: "decision must be APPROVE or REJECT" })
      };
    }
    ...
    if (res.Item.status !== "PENDING") {
      return {
        statusCode: 409,
        body: JSON.stringify({ message: `Already ${res.Item.status}` })
      };
    }
    ...
    if (!taskToken) {
      return {
        statusCode: 409,
        body: JSON.stringify({
          message: "No taskToken on item. NotifyAdminFn did not store it yet."
        })
      };
    }
    await sfn.send(
