...
var SCENE_PRODUCT_MAP_TABLE = process.env.SCENE_PRODUCT_MAP_TABLE_NAME;
...
var handler = async (event) => {
  console.log("getShopThisScene called", JSON.stringify(event, null, 2));
  const sceneId = event.sceneId ?? event.arguments?.sceneId ?? null;
  if (!sceneId) {
    throw new Error("sceneId is required");
  }
  const mapRes = await ddb.send(
    new import_lib_dynamodb.QueryCommand({
      TableName: SCENE_PRODUCT_MAP_TABLE,
