...
async function handleAdminListPending(
  ...
  // Only ADMIN/PRIME can list pending items
  const tier = getUserTier(identity);
...
async function handleAdminListClosetItems(
  args: { status?: string; limit?: number; nextToken?: string },
  identity: SAIdentity
) {
  const tier = getUserTier(identity);
  if (tier !== "ADMIN" && tier !== "PRIME") {
...
async function handleAdminListBestieClosetItems(
  args: { status?: string; limit?: number; nextToken?: string },
  identity: SAIdentity
) {
  const tier = getUserTier(identity);
  ...
  // Filter for BESTIE+ users
  const res = await ddb.send(
    new QueryCommand({
      TableName: TABLE_NAME,
      IndexName: STATUS_GSI,
      KeyConditionExpression: "#status = :status",
      FilterExpression: "contains(#tier, :bestieTier)",
      ExpressionAttributeNames: { "#status": "status", "#tier": "userTier" },
      ExpressionAttributeValues: {
        ":status": { S: status },
        ":bestieTier": { S: "BESTIE" },
      },
...
async function handleAdminCreateClosetItem(
  args: { input: any },
  identity: SAIdentity
) {
  const tier = getUserTier(identity);
  ...
  const item = {
    PK: { S: `USER#${args.input.userId}` },
    SK: { S: `CLOSET#${itemId}` },
    id: { S: itemId },
    userId: { S: args.input.userId },
    ownerSub: { S: args.input.userId },
    title: { S: args.input.title || "Untitled" },
    status: { S: args.input.status || "PUBLISHED" },
    audience: { S: args.input.audience || "PUBLIC" },
    createdAt: { S: now },
    updatedAt: { S: now },
    createdBy: { S: identity?.sub || "admin" },
    ...ddbMarshal(args.input.metadata || {}),
  };
...
async function handleAdminApproveItem(
  args: { closetItemId: string },
  identity: SAIdentity
) {
  const tier = getUserTier(identity);
  ...
  const res = await ddb.send(
    new UpdateItemCommand({
      TableName: TABLE_NAME,
      Key: { PK: { S: `CLOSET#${args.closetItemId}` }, SK: { S: "ITEM" } },
      UpdateExpression:
        "SET #status = :approved, #updated = :now, #approvedBy = :approver",
      ExpressionAttributeNames: {
        "#status": "status",
        "#updated": "updatedAt",
  ...
  await eb.send(
    new PutEventsCommand({
      Entries: [
        {
          Source: "closet.admin",
          DetailType: "ItemApproved",
          Detail: JSON.stringify({
            itemId: args.closetItemId,
...
async function handleAdminRejectItem(
  args: { closetItemId: string; reason?: string },
  identity: SAIdentity
) {
  const tier = getUserTier(identity);
  if (tier !== "ADMIN" && tier !== "PRIME") {
  ...
  const res = await ddb.send(
    new UpdateItemCommand({
      ...
      Key: { PK: { S: `CLOSET#${args.closetItemId}` }, SK: { S: "ITEM" } },
      UpdateExpression:
        "SET #status = :rejected, #updated = :now, #rejectedBy = :rejecter, #rejectionReason = :reason",
      ExpressionAttributeNames: {
        "#status": "status",
        "#updated": "updatedAt",
      ...
      ExpressionAttributeValues: {
        ":rejected": { S: "REJECTED" },
        ":now": { S: now },
        ":rejecter": { S: identity?.sub || "admin" },
        ":reason": { S: args.reason || "No reason provided" },
      },
      ReturnValues: "ALL_NEW",
    })
  );
...
async function handleAdminPublishClosetItem(
  args: { closetItemId: string },
  identity: SAIdentity
) {
  const tier = getUserTier(identity);
  ...
  const res = await ddb.send(
    new UpdateItemCommand({
      TableName: TABLE_NAME,
      Key: { PK: { S: `CLOSET#${args.closetItemId}` }, SK: { S: "ITEM" } },
      UpdateExpression: "SET #status = :published, #updated = :now",
      ExpressionAttributeNames: {
        "#status": "status",
        "#updated": "updatedAt",
      },
...
async function handleAdminSetClosetAudience(
  args: { closetItemId: string; audience: string },
  identity: SAIdentity
) {
  const tier = getUserTier(identity);
  if (tier !== "ADMIN" && tier !== "PRIME") {
  ...
  const res = await ddb.send(
    new UpdateItemCommand({
      TableName: TABLE_NAME,
      Key: { PK: { S: `CLOSET#${args.closetItemId}` }, SK: { S: "ITEM" } },
      UpdateExpression: "SET #audience = :audience, #updated = :now",
      ExpressionAttributeNames: {
        "#audience": "audience",
        "#updated": "updatedAt",
      },
...
async function handleAdminUpdateClosetItem(
  args: { input: any },
  identity: SAIdentity
) {
  const tier = getUserTier(identity);
  ...
  if (args.input.title) {
    ...
    names["#title"] = "title";
    values[":title"] = { S: args.input.title };
  }

  if (args.input.status) {
    updates.push("#status = :status");
    names["#status"] = "status";
    values[":status"] = { S: args.input.status };
  }

  if (args.input.audience) {
    updates.push("#audience = :audience");
    names["#audience"] = "audience";
    values[":audience"] = { S: args.input.audience };
  }
  ...
  const res = await ddb.send(
    new UpdateItemCommand({
      TableName: TABLE_NAME,
      Key: {
        PK: { S: `CLOSET#${args.input.closetItemId}` },
        SK: { S: "ITEM" },
      },
...
async function handleAdminListBackgroundChangeRequests(
  args: { limit?: number; nextToken?: string },
  identity: SAIdentity
) {
  const tier = getUserTier(identity);
  ...
  const res = await ddb.send(
    new QueryCommand({
      TableName: TABLE_NAME,
      IndexName: STATUS_GSI,
      KeyConditionExpression: "#type = :type AND #status = :pending",
      ExpressionAttributeNames: { "#type": "type", "#status": "status" },
      ExpressionAttributeValues: {
        ":type": { S: "BG_CHANGE_REQUEST" },
        ":pending": { S: "PENDING" },
      },
      Limit: limit,
      ExclusiveStartKey: args.nextToken
...
async function handleAdminApproveBackgroundChange(
  ...
  const res = await ddb.send(
    new UpdateItemCommand({
      TableName: TABLE_NAME,
      Key: { PK: { S: `BG_CHANGE#${args.requestId}` }, SK: { S: "REQUEST" } },
      UpdateExpression: "SET #status = :approved, #updated = :now",
...
async function handleAdminRejectBackgroundChange(
  ...
  const res = await ddb.send(
    new UpdateItemCommand({
      TableName: TABLE_NAME,
      Key: { PK: { S: `BG_CHANGE#${args.requestId}` }, SK: { S: "REQUEST" } },
      UpdateExpression: "SET #status = :rejected, #updated = :now, #reason = :reason",
      ExpressionAttributeNames: {
        "#status": "status",
        "#updated": "updatedAt",
      ...
      ExpressionAttributeValues: {
        ":rejected": { S: "REJECTED" },
        ":now": { S: now },
        ":reason": { S: args.reason || "Rejected by admin" },
      },
      ReturnValues: "ALL_NEW",
    })
  );
  ...
  try {
    switch (fieldName) {
      ...
      case "adminListPending":
        return await handleAdminListPending(event.arguments || {}, event.identity);

      case "adminListClosetItems":
        return await handleAdminListClosetItems(event.arguments || {}, event.identity);

      case "adminListBestieClosetItems":
        return await handleAdminListBestieClosetItems(event.arguments || {}, event.identity);
      ...
      case "adminCreateClosetItem":
        return await handleAdminCreateClosetItem(event.arguments || {}, event.identity);

      case "adminApproveItem":
        return await handleAdminApproveItem(event.arguments || {}, event.identity);

      case "adminRejectItem":
        return await handleAdminRejectItem(event.arguments || {}, event.identity);

      case "adminPublishClosetItem":
        return await handleAdminPublishClosetItem(event.arguments || {}, event.identity);

      case "adminSetClosetAudience":
        return await handleAdminSetClosetAudience(event.arguments || {}, event.identity);

      case "adminUpdateClosetItem":
        return await handleAdminUpdateClosetItem(event.arguments || {}, event.identity);

      case "adminListBackgroundChangeRequests":
        return await handleAdminListBackgroundChangeRequests(event.arguments || {}, event.identity);

      case "adminApproveBackgroundChange":
        return await handleAdminApproveBackgroundChange(event.arguments || {}, event.identity);

      case "adminRejectBackgroundChange":
        return await handleAdminRejectBackgroundChange(event.arguments || {}, event.identity);
