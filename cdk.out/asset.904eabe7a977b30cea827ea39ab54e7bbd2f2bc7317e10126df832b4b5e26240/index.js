...
// lambda/closet/approve-closet-upload.ts
...
var handler = async (event) => {
  try {
    ...
    const pk = `APPROVAL#${approvalId}`;
    const res = await ddb.send(
      new import_lib_dynamodb.GetCommand({
        TableName: TABLE_NAME,
        Key: { [PK_NAME]: pk, [SK_NAME]: "REQUEST" }
      })
    );
    if (!res.Item) return { statusCode: 404, body: JSON.stringify({ message: "Approval not found" }) };
    ...
    await ddb.send(
      new import_lib_dynamodb.UpdateCommand({
        TableName: TABLE_NAME,
        Key: { [PK_NAME]: pk, [SK_NAME]: "REQUEST" },
        UpdateExpression: "SET #status = :s, decidedAt = :t",
