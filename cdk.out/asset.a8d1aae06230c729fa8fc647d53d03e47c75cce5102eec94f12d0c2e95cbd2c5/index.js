...
var handler = async (event) => {
  try {
    ...
    if (event.httpMethod === "POST" && event.path.endsWith("/presign")) {
      ...
      const url = await (0, import_s3_request_presigner.getSignedUrl)(s3, new import_client_s3.PutObjectCommand({ Bucket: BUCKET, Key, ContentType: contentType }), { expiresIn: 900 });
      return {
        statusCode: 200,
        headers: { "Content-Type": "application/json" },
