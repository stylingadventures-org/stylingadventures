...
/* ---------- helpers for uploads/thumbs ---------- */
// If the S3 object key already begins with "thumbs/", use as-is.
// Otherwise map to the thumb location ("thumbs/<original-key>" or ".jpg" variant depending on your Lambda).
function toThumbKey(originalKey) {
  const k = decodeURIComponent(originalKey || '');
  if (k.startsWith('thumbs/')) return k;
  // Your Lambda writes thumbs under "thumbs/<original key>" (Option A you picked).
  return `thumbs/${k.replace(/^\/+/, '')}`;
}
...
function App() {
  ...
  React.useEffect(() => {
    (async () => {
      try {
        const c = await (await fetch('/config.json?ts=' + Date.now(), { cache: 'no-store' })).json();

        // Normalize uploads API base; support either uploadsApiUrl or uploadsUrl key
        const base = (c.uploadsApiUrl || c.uploadsUrl || '').trim();
        if (!base) throw new Error('config.json missing "uploadsApiUrl"');
        c._uploadsBase = base.endsWith('/') ? base : base + '/';

        // Optional CloudFront for thumbs (prefer explicit thumbsCdn, otherwise same origin)
  ...
  // --- Upload + List demo (kept from your original) ---
  async function presignAndUpload() {
    ...
    try {
      ...
      const pre = await fetch(cfg._uploadsBase + 'presign', {
        ...
        body: JSON.stringify({ key: uploadKey, contentType: inferMime(uploadKey) })
      }).then(async r => (r.ok ? r.json() : Promise.reject(await r.text())));

      if (!pre?.url) throw new Error('Presign response missing url');

      // 2) upload sample content or blob if you have a file picker
      const body = isText(uploadKey) ? 'hello from the browser' : new Uint8Array([0xFF, 0xD8, 0xFF]);
      await fetch(pre.url, {
        method: 'PUT',
        headers: { 'Content-Type': inferMime(uploadKey) },
        body
      }).then(async r => (r.ok ? r : Promise.reject('S3 PUT failed ' + r.status)));

      // 3) list (populate gallery)
      await listUploads();
    } catch (err) { setError(String(err)); }
    finally { setBusy(false); }
  }

  // --- List objects (fills `items`) ---
  async function listUploads() {
    ...
    try {
      ...
      const keys = (lst.items || []).map(i => i.key || i); // support {key} or plain string
      setItems(keys);
      // kick off thumb checks
      keys.forEach(k => waitForThumb(k));
    } catch (err) { setError(String(err)); }
    finally { setBusy(false); }
  }

  // --- Poll /thumb-head?key=<original-or-thumb-key> until ready ---
  async function waitForThumb(originalKey) {
    if (!originalKey) return;
    const thumbKey = toThumbKey(originalKey);
    let tries = 0;
    while (tries++ < 20) { // ~20 * 750ms = 15s max
      try {
    ...
    // give up (still show placeholder broken img; user can refresh)
  ...
  // Legacy alias so old markup like onclick="startUpload()" still works
  React.useEffect(() => { window.startUpload = presignAndUpload; window.listUploads = listUploads; }, [cfg, uploadKey]);
        ...
        items.map((orig) => {
          ...
          return e('figure', {
            ...
            e('img', {
              ...
              onError: () => { /* let it show broken until thumb exists */ }
            }),
