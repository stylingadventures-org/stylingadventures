...
var ddb = import_lib_dynamodb.DynamoDBDocumentClient.from(new import_client_dynamodb.DynamoDBClient({}), {
  marshallOptions: {
    ...
    // avoids "undefined" silently dropping fields
  }
});
...
var handler = async (event) => {
  ...
  if (!approvalId) throw new Error("Could not determine approvalId from event.item (expected item.id)");
  ...
  console.log("[notify-admin] ddb update result (ALL_NEW):", JSON.stringify(result.Attributes));
