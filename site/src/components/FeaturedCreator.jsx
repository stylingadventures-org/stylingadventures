      ...
      <div className="relative z-10">
        ...
        <div className="flex items-center gap-4 mb-4">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              <h3 className="text-lg font-bold text-gray-900">{creator.displayName || creator.name}</h3>
              <span className="px-2 py-0.5 bg-purple-100 text-purple-700 text-xs font-semibold rounded-full">
                {creator.tier || 'creator'}
              </span>
            </div>
            <p className="text-sm text-gray-600">@{creator.handle || creator.displayName?.toLowerCase().replace(/\s+/g, '')}</p>
          </div>
        </div>

        {/* Bio excerpt */}
        <p className="text-gray-700 text-sm mb-3 line-clamp-2">{creator.bio || 'Creating amazing content'}</p>
        ...
        <div className="grid grid-cols-3 gap-4 mb-4 py-3 bg-white rounded-lg">
          <div className="text-center">
            <p className="font-bold text-gray-900">{(creator.followers || creator.stats?.followers || 0).toLocaleString()}</p>
            <p className="text-xs text-gray-600">Followers</p>
          </div>
          <div className="text-center">
            <p className="font-bold text-gray-900">{(creator.posts || creator.stats?.posts || 0).toLocaleString()}</p>
            <p className="text-xs text-gray-600">Posts</p>
          </div>
          <div className="text-center">
            <p className="font-bold text-gray-900">{(creator.tracks || creator.stats?.tracks || 0).toLocaleString()}</p>
